...
import concurentDocs.service.actor.CollabRoomActor.CollabRoomEvent.CollabError
import concurentDocs.service.actor.CollabRoomActor.CollabRoomEvent.UserLogout
...
object EditorFlow {
  ...
  def getEditorFlow(
      user: User,
      flowRequestorActor: ActorRef[CollabRoomEvent],
      chatRoomActor: ActorRef[CollabRoomEvent]
  )(implicit log: Logger): Flow[FrontendMessage, FrontendMessage, _] = {

    val completionMatcher: PartialFunction[CollabRoomEvent, Unit] = { case UserLogout(user) =>
      log.info(s"[EditorFlow - $user] Completing flow with user logout")
    }

    val failureMatcher: PartialFunction[CollabRoomEvent, Throwable] = { case CollabError(e) =>
      new RuntimeException(e.toString)
    }

    Flow.fromGraph(
      GraphDSL.createGraph(
        ActorSource.actorRef[CollabRoomEvent](
          bufferSize = 100,
      ...
      ) { implicit builder => editorSource: SourceShape[CollabRoomEvent] =>
        ...
        val fromEditor: FlowShape[FrontendMessage, CollabRoomEvent] = builder.add(
          Flow[FrontendMessage].collect {
            ...
            case msg @ TextEditorDomain.EditorContent(content, deltas) => CollabRoomEvent.EditorContent(content, deltas.ops, user)
          }
        )
        ...
        val backToEditor: FlowShape[CollabRoomEvent, FrontendMessage] = builder.add(
          Flow[CollabRoomEvent].collect {
            ...
            case ContentRequest(_) => TextEditorDomain.ContentRequest
